#!/usr/bin/env bash
# E2E: one fmcp serve session — gforth_eval timeout then mcp_ping (no restart).
set -euo pipefail
...
out=$(printf '%s\n' \
  ...
  '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gforth_eval","arguments":{"project_root":"'"$ROOT"'","source":"10000 ms","timeout_seconds":1}}}' \
...
echo "mcp_session_timeout_test OK"
