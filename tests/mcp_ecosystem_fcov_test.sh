#!/usr/bin/env bash
# E2E: fcov_run for all feco repos in one serve session, then mcp_ping.
set -euo pipefail
FMCP_HOME="$(cd "$(dirname "$0")/.." && pwd)"
WS="${FECO_WORKSPACE:-$(dirname "$FMCP_HOME")}"
export FMCP_HOME PATH="$FMCP_HOME/bin:$PATH"

repos=(frules fsemver ttester fenum f fmix flint fcov fmcp)
lines=(
...
echo "mcp_ecosystem_fcov_test OK"
